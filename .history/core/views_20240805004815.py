...
def vendasMesTotal():  
    ...
    return({'total_mes': total_mes, 'vendas_mensais': vendas_mensais, 'primeiro_ano': primeiro_ano, 'primeiro_mes': primeiro_mes})
...
class DetalhesDias(TemplateView):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {'vendas_dia':vendas_dia, 'total_dia':total_dia,
            'total_dia': total_dia,
            'total_semana': vendasSemana,
            'total_mes': vendasMes,
            'vendas_mensais': vendasMes, 
            'totalVendas':totalVendas,
            'mais_vendido' : maisvendido})

class DetalhesSemana(TemplateView):
    template_name = 'detalhesSemana.html'
    def get(self, request):
        # Obtém a data atual
        hoje = datetime.now().date()
        ...
        inicio_semana = hoje - timedelta(days=hoje.weekday())
        vendas_semana = VendaDoProduto.objects.filter(data_venda__gte=inicio_semana, data_venda__lte=hoje)
        return render(request, self.template_name, {'vendasSemana':vendasSemana,
...
class DetalhesMes(TemplateView):
    template_name = 'detalhesMes.html'
    def get(self, request):
        # Obtém a data atual
        hoje = datetime.now().date()
        ...
        inicio_mes = hoje.replace(day=1)
        vendas_mes = VendaDoProduto.objects.filter(data_venda__gte=inicio_mes, data_venda__lte=hoje)
        return render(request, self.template_name, {'vendas_mes' : vendas_mes , 'vendasSemana':vendasSemana,
                                                    'total_dia': vendasDia, 
                                                    'total_mes': vendasMes,
                                                    'totalVendas':totalVendas,
                                                    'mais_vendido' : maisvendido})
