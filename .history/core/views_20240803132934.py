...
class Index(TemplateView):
    ...
    def get(self, request):
        ...
        produtos_vendidos = VendaDoProduto.objects.values('produto__nome_produto').annotate(total_vendido=Sum('qtd')).order_by('-total_vendido')
        
        # Obtém o produto mais vendido
        mais_vendido = produtos_vendidos.first() if produtos_vendidos else None

        return render(request, self.template_name, {'vendas': vendas, 'total': total, mais_vendido})  